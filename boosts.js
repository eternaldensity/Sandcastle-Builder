...
Molpy.DefineBoosts = function() {
	...
	new Molpy.Boost({
		...
		icon: 'bkj',
		group: 'hpt',
		desc: 'Antalet redundanta klickade kattungar läggs till blixtnedslag multiplikator.',
		Sand: '9.8M',
		Castles: 888555222,
	...
	Molpy.RiftJump = function() {
		...
		if(Molpy.Got('Temporal Rift')) {
			if(Molpy.Got('Safety Net'))
				Molpy.newpixNumber = Math.round(Math.random() * (Molpy.highestNPvisited - 241) + 241)
			else
	...
	new Molpy.Boost({
		...
		desc: function(me) {
			if(!me.bought)
				return 'Allows you to change the number of times Automata Assemble tries to run Factory Automation after Tool Factory.<br>(Otherwise it defaults to the level from Production Control)';
			var n = me.Level;
	...
	new Molpy.Boost({
		...
		desc: function(me) {
			...
			if(n < Molpy.Boosts['PC'].power) {
				str += '<br><input type="Button" value="Increase" onclick="Molpy.ControlAutomata(20,1)"></input> the number of runs by 20 at a cost of '
					+ Molpify(logicatCost) + ' Logicat Levels and ' + Molpify(pageCost, 2) + ' Blackprint Pages.';
			} else {
				str += '<br>It will cost ' + Molpify(logicatCost) + ' Logicat Levels and '
					+ Molpify(pageCost, 2) + ' Blackprint Pages to increase this by 20.';
			}
			return str;
		},
	...
	new Molpy.Boost({
		...
		desc: function(me) {
			...
			if(me.bought) {
				...
				str += '<br><input type="Button" onclick="if(Molpy.Spend({FluxCrystals:' + p
					+ '}))Molpy.Add(\'Time Lord\',0,1);" value="Pay"></input> ' + Molpify(p, 1)
					+ ' Flux Crystals to increase this by 1.';
			}
			return str;
		}
	});
