...
{	
	...
	new Molpy.Badge({name:'Typo Storm',desc:'Encounter 1000 different typos in a play session (resets on reload)',stats:'Toggle typo mode by typing "typo" into the Import dialog.<br>The number of typos are limitted by the NewPix you are on.<br>Actual typos the the developers don\'t count.<br>For instance, "limitted".<br>Sorry.'});
		...
		new Molpy.Badge({name:Molpy.groupNames[kind][3]+': '+(args.np<0?'Minus ':'')+args.name,alias:kind+args.np,np:args.np,
			desc:function(me)
			{
				...
				if(me.group=='discov')
				{
					...
					if(Molpy.Got('SMM')&&!(Molpy.Boosts['SMM'].power||Molpy.Boosts['SMF'].power&&Molpy.Boosts['SMF'].bought==me.np)&&!Molpy.Earned('monums'+me.np))
					{
				...
				{
					if(Molpy.Got('GMM')&&!(Molpy.Boosts['GMM'].power||Molpy.Boosts['GMF'].power&&Molpy.Boosts['GMF'].bought==me.np)&&!Molpy.Earned('monumg'+me.np))
					{
